...
params <- list(n=250, # number of observations
               ...
               cov_type= c("MGM"), # specifing distributional form from which data are generated (MGM: mixed graphical model)
               MGM.max.degree=c(1:5)*10, # maximum number of connections in graphical model (relevant only if cov_type=="MGM")
...
saveRDS(results.global, file="output/Figure4/resultsglobal.RDS") # save results to file
...
gg.dat <- results.global %>%
  mutate(method=factor(ifelse(knockoff.method=="seq",
                              "(vanilla) sequential knockoffs",
                              ifelse(knockoff.method=="sparseseq",
                                     "sparse sequential knockoffs",
                                     knockoff.method)
  ),
  levels=c( "second-order MX knockoffs", "(vanilla) sequential knockoffs", "sparse sequential knockoffs"))
  ) # make knockoff method labels easier to read and order them as desired


## create first subplot
pp1 <- ggplot(data=gg.dat, aes(x=as.numeric(MGM.max.degree), y=as.numeric(TPP), group=method, colour=method)) +
  xlab("") +
  ylab("TDP") +
...
pp2 <- ggplot(data=gg.dat, aes(x=as.numeric(MGM.max.degree), y=as.numeric(FDP), group=method, colour=method)) +
  xlab("maximum number of connections per node in graphical model") +
  ylab("FDP") +
  stat_summary(fun.data = "mean_se", aes(colour=method), size=0.2) +
  stat_summary(fun.data = "mean_se", aes(colour=method, group=method), geom="line", size=0.2) +
  theme(legend.position="bottom", legend.text=element_text(size=12), legend.title=element_text(size=12)) +
...
## create thrid subplot
pp3 <- ggplot(data=gg.dat, aes(x=as.numeric(MGM.max.degree), y=as.numeric(time), group=method, colour=method)) +
  xlab("") +
...
fig4B <- grid_arrange_shared_legend(pp1, pp2, pp3) 
ggsave(file="output/Figure4/Figure4B.pdf", plot=fig4B, width=12, height=4) ## save to file (pdf)
ggsave(file="output/Figure4/Figure4B.png", plot=fig4B, width=12, height=4) ## save to file (png)
